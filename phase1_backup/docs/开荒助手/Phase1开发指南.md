...
**时间：** 第10-13周  
**状态：** ✅ **Phase 1 已圆满完成 (2026-04-16)**
**交付物：** 
- ✅ 可运行的 Python 命令行程序 (`main.py`)
...
**交互设计规范：**
*   **菜单驱动：** 采用清晰的数字菜单，引导用户完成注册、记录添加和 AI 咨询。
*   **输入校验：** 对所有数值型输入（年龄、体重）进行 `try-except` 捕获，防止非数字输入导致崩溃。
*   **封装原则：** 严禁在 `main.py` 中直接访问底层数据结构（如 `profile_manager.profiles`），所有状态获取必须通过公开接口（如 `system.get_pet_count()`）。
*   **优雅降级：** AI 调用被包裹在异常处理中，即使本地 LLM 服务未启动，程序也会给出友好提示而非闪退。
